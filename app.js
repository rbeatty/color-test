...
bg = document.getElementById("bg-slider-input"),
bb = document.getElementById("bb-slider-input"),
bh = document.getElementById("bh-slider-input"),
bs = document.getElementById("bs-slider-input"),
bv = document.getElementById("bv-slider-input"),
fr = document.getElementById("fr-slider-input"),
fg = document.getElementById("fg-slider-input"),
fb = document.getElementById("fb-slider-input"),
fh = document.getElementById("fh-slider-input"),
fs = document.getElementById("fs-slider-input"),
fv = document.getElementById("fv-slider-input");
...
// INIT INITIAL COLOURS - grab initial colours from hash
var hashBg = location.hash.split('/')[1],
hashFg = location.hash.split('/')[0];

fore.value = hashFg || '5AFFFF';
fore.onchange();

back.value = hashBg || '743333';
back.onchange();
...
function updateColourResults() {
	document.body.style.color = "rgb(" + fr.value + "," + fg.value + "," + fb.value + ")";
	document.body.style.backgroundColor = "rgb(" + br.value + "," + bg.value + "," + bb.value + ")";
	...
	// colorThreshold = 500,
	// bY = ((br.value * 299) + (bg.value * 587) + (bb.value * 114)) / 1000,
	// fY = ((fr.value * 299) + (fg.value * 587) + (fb.value * 114)) / 1000,
	// brightnessDifference = Math.abs(bY-fY),
	// colorDifference = (Math.max (fr.value, br.value) - Math.min (fr.value, br.value)) + (Math.max (fg.value, bg.value) - Math.min (fg.value, bg.value)) + (Math.max (fb.value, bb.value) - Math.min (fb.value, bb.value));
	...
	document.getElementById('contrastratio').innerHTML = ratio;
	...
	if (ratio >= 7) {
		...
		document.getElementById('level').innerHTML = 'AA<small>(18px+)</small>';
	} else {
